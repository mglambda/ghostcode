...
class EndInteractionPart(BaseModel):
    ...
    reason: str = Field(
        description = "The reason for ending the interaction."
    )

    def show_cli(self) -> str: return show_model(self)
    def show(self) -> str: return show_model(self)
...
class ActionShellCommand(BaseModel):
    ...
    def run(self, tty: shell.VirtualTerminal, clearance: Optional[ClearanceRequirement] = None) -> None:
        ...
        if clearance is None:
            raise RuntimeError("Bad call of ActionShellCommand.run: You **must** provide the clearance parameter explicitly.")
