...
class IsBackendQuery(Protocol):
    """Protocl for anything that supports LLM query like properties.
    This applies to e.g. ActionQueryCoder, ActionQueryWorker, but also their preceding actions, like ActionPrepareCoderQuery."""

    prompt: str
    preamble_config: PromptConfig
    llm_response_profile: LLMResponseProfile
...
class CodeResponsePart(BaseModel):
    ...
    **Crucially, for 'partial' edits, the 'original_code' field MUST be accurately provided.**
    ...
    for 'original_code', the partial replacement operation will fail, potentially leading
    to incorrect file modifications or full file replacements.

    **Important Guidance for LLM Generation:**
    1.  **Single Edit Per File:** When making changes to a file, generate only *one* `CodeResponsePart` per file per turn. If multiple changes are needed in the same file, generate the first change, and then indicate that further changes are required in a `TextResponsePart` so the system can re-evaluate the file before the next change. You may ignore this rule in large files where edits are many (100+) lines apart and do not overlap.
    2.  **Context for `original_code`:** For `type: "partial"`, ensure the `original_code` block includes at least 5-10 lines of surrounding context (both before and after the actual change) to aid in precise location within the file. The more context, the better the match reliability.
    3.  **Use `context_anchor`:** Always provide a `context_anchor` for `partial` `CodeResponsePart`s. This anchor should be a unique and stable identifier of the containing code structure (e.g., `def my_function(arg):`, `class MyClass:`, `# Main loop comment`). This significantly improves the chances of correctly locating the target code.
    4.  **Prefer `full` for Small Files:** For files under approximately 100 lines, prefer generating `CodeResponsePart`s of `type: "full"` to replace the entire file, as this is generally more robust than partial edits for small files.
...
class TextResponsePart(BaseModel):
    ...
    Note to ghostcoder: When generating code, use this as the first response part to give the user a confirmation and short indication of what you're about to do.
...
class CoderResponse(BaseModel):
    """A response returned from the ghostcoder code generation backend.
...
class UserInteractionHistoryItem(BaseModel):
    ...
    def set_git_commit_hash(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        current Git commit hash.
        """
        # FIXME: remove this at some point, preamble is deprecated and we want compatibility with older versions
...
class Project(BaseModel):
    ...
    nag_sources: List[NagSource] = Field(
        default_factory = list,
