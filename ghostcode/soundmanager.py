...
class SoundManager:
    ...
    sound_directory: str = field(
        metadata={
            "description": "The directory where .wav files are found. All files in that directory are initialized into the internal dictionary. To play a sound, call the various play methods with the filename of the wav file, including the extension, but without any preceding path."
        }
    )
    ...
    volume_multiplier: float = field(
        default=1.0,
        metadata={
            "description": "Adjust global volume by this factor."
        },  # FIXME: Requires numpy for actual audio data manipulation.
    )
    ...
    sounds: Dict[str, str] = field(  # Stores filename -> full_filepath
        ...
        metadata={
            "description": "Repository of indexed sound files. These are loaded from the sound_directory and initialized as playable sound objects."
        },
    )
    ...
    stop_playback_flag: Event = field(
        ...
        metadata={
            "description": "Event that signals all playback to stop and playback threads to exit gracefully."
        },
    )
    ...
    _active_playback_threads: List[Thread] = field(
        ...
        init=False,
        metadata={
            "description": "List of currently active playback threads (for individual sounds)."
        },
    )
    ...
    def _load_sounds(self) -> None:
        ...
        if not os.path.isdir(self.sound_directory):
            logger.warning(
                f"Sound directory '{self.sound_directory}' does not exist. No sounds loaded."
            )
            return
    ...
    def _start_playback_thread(self, sound_filename: str) -> None:
        ...
        if sound_filepath is None:
            logger.warning(
                f"Sound '{sound_filename}' not found in repository. Cannot start playback."
            )
            return

        # Clean up dead threads before adding a new one
        self._active_playback_threads = [
            t for t in self._active_playback_threads if t.is_alive()
        ]
    ...
    def play_loop(
        ...
        if not self.sound_enabled:
            logger.debug(
                f"Sound playback disabled. Skipping play_loop('{sound_filename}')."
            )
            return
        ...
        if sound_filepath is None:
            logger.warning(
                f"Sound '{sound_filename}' not found in repository. Cannot start looped playback."
            )
            return

        def loop_worker() -> None:
            current_repetition = 0
            while not self.stop_playback_flag.is_set() and (
                repetition_number == -1 or current_repetition < repetition_number
            ):
            ...
            logger.debug(
                f"Finished looped playback for {sound_filename} after {current_repetition} repetitions."
            )
    ...
    def continuous_playback(
        self,
        random_sound_filenames: List[str],
        mean: float = 0.8,
        ...
        if not random_sound_filenames:
            logger.warning(
                "No sound filenames provided for continuous playback. Skipping."
            )
            yield
            return
        ...
        if not playable_sounds:
            logger.warning(
                "No playable sounds found in repository for continuous playback. Skipping."
            )
            yield
            return
