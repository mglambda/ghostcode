...
class ProgressPrinter:
    ...
    def _print_spinner(self) -> None:
        ...
        while self._is_running.is_set():
            self.print_function(
                f"\r{self.message}{spinner_chars[i % len(spinner_chars)]} ",
                end="",  # type: ignore
                flush=True,
            )
            i += 1
    ...
    def _print_dots(self) -> None:
        ...
        while self._is_running.is_set():
            self.print_function(
                f"\r{self.message}{dot_patterns[i % len(dot_patterns)]}",
                end="", # type: ignore
                flush=True,
            )
            i += 1
    ...
    def start(self) -> None:
        ...
        target = style_to_target.get(
            self.style, self._print_dots
        )  # Default to dots if style is unknown
    ...
    def stop(self) -> None:
        ...
        self.print_function(
            "\r" + " " * 4 + "\r", end="", flush=True # type:ignore
