    ...
    def run(self, prog: Program) -> CommandOutput:
        ...
        if self.interaction_identifier:
            ...
            if len(found_by_id) == 1:
                ...
                result.print(f"--- Interaction Details (ID: {interaction.unique_id}, Tag: {interaction.tag}) ---")
                result.print(interaction.show())
                return result
            elif len(found_by_id) > 1:
                # This should ideally not happen for unique_id, but handle defensively
                result.print(f"CRITICAL ERROR: Multiple interactions found with the same unique ID '{target_id}'. This indicates data corruption.")
                return result
            ...
            if len(found_by_tag) == 1:
                ...
                result.print(f"Multiple interactions found with tag '{target_id}'. Please use the full unique ID for detailed view:")
                for interaction in found_by_tag:
                    result.print(f"  - ID: {interaction.unique_id}, Tag: {interaction.tag}, Title: '{interaction.title}', Turns: {len(interaction.contents)}")
                return result
            else:
            ...
            if not project.interactions:
                ...
                for interaction in reversed(project.interactions):
                    ...
                    result.print(self._overview_show_interaction(interaction, num_turns))
        return result
...
def main():
    ...
    log_parser.add_argument("--interaction", type=str,
                            help="Display a specific interaction in detail by its unique ID or tag.")
    log_parser.set_defaults(func=lambda args: LogCommand(interaction_identifier=args.interaction))
