...
Emacs context file overview buffer
Emacs bootstrap (ghostcode.el which is loaded via emacsclient).

Make it so that pasted interaction IDs cause a rag lookup.
* todo
** general
Addd FileRequestParts or something (ghost is starting to request files on its own).
cached_token_cost on contextfileconfig.
investigate how to use treesitter to build code graphs which we can use to more efficiently build prompt context
Investigate prompt_toolkit to fix double input problem.
paranoia: Are the LLMs hallucinating timestamps? (investigate and solve with a model_validator)
...
Probably use sqlite to save state like history, contexte etc.
...
** interact
** nag
Fix bonkers hallucinations during speaker handling of user requests.
...
Use an emacs buffer as a shell for ghostcoder (this is very powerful since shell emulation is hard)
...
** worker
Improve context expansion by worker
...
get rid of routing.
fix bad string for context wipe
Implement logic of reduce token action
Make preamble be set in query coder action not in interact subcommand.
give more context to token cost evaluation in worker.
Investigate coder horribly messing up code parts - i never know if it's me or google
Make queueing actions in bulk possible.
investigate summaries never filling in depends_on field.
add --coder-model and --worker-model as CLI parameters
Remove depends_on field
Make it so that last request in interact session is automatically put into emacs kill ring.
Emacs region as a source in prompts.
Emacs region as an action target.
Make project and prog.read_lock only source of truth about current interaction.
Make nag send requests using magic trigger words.
Fix filtering of magic nag_transcribe string.
Fix levenshtein distance calculation (wtf)
Add --last to ghostcode log and ghostcode interact and ghostcode talk to print/resume the latest interaction.
Fix input() scheme (we have double input with interact loop and possible voice initiated file edits)
Audio input support for small queries ("hey ghostcode, what was the parser class called in this project?") (nag)
Add /help command.
